...
export default function PayPage() {
  ...
  useEffect(() => {
    ...
    if (billIdParam) {
      ...
      const tAddr = token === 'USDC' ? TOKENS.USDC
        ...
        : token === 'MNT' || token === 'ETH' ? TOKENS.ETH
        : ''
  ...
  const executePayment = async () => {
    ...
    try {
      ...
      // Ensure correct chain to avoid MetaMask circuit breaker on wrong network
      const targetChainIdHex = `0x${ARBITRUM_SEPOLIA.id.toString(16)}`
      try {
        ...
        setUiError('Please approve network switch to Arbitrum Sepolia and try again.')
        return
      }
      ...
      const amount = billDetails?.token === TOKENS.ETH ? paymentData?.amount : undefined
  ...
  const isGaslessFlow = (() => {
    ...
    if (auth) return tokenAddress && tokenAddress !== TOKENS.ETH
    // Else, if we have parsed symbol
    if (paymentData?.token) return paymentData.token !== 'ETH' && paymentData.token !== 'MNT'
    // Else, if bill details loaded
    if (billDetails?.token) return billDetails.token !== TOKENS.ETH
    return false
  })()
                        ...
                        >
                          {copiedLink ? <Check className="w-4 h-4 text-green-600"/> : <>
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
                              <path d="M16 1H4c-1.1 0-2 .9-2 2v12h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h10c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h10v14z"/>
                            </svg>
                          </>}
                        </Button>
                      </div>
